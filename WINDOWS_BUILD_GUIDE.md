...
2. Выполните команду:
   ...
   dotnet --version
   ```
3. Вы должны увидеть версию **8.0.xxx**. Например, `8.0.123` или более новую.

> ⚠️ Если вы видите другую версию (например `8.0.418`), то:
> * либо **установите ту же версию SDK**, что указана в `global.json` проекта;
> * либо **обновите `global.json`** до вашей установленной версии, чтобы команды `dotnet workload` работали.
>   Пример изменения:
>   ```json
>   {
>     "sdk": { "version": "8.0.418" }
>   }
>   ```
>   После правки запустите `dotnet restore` снова.
